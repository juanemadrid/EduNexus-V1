...
import { TIPOS_IDENTIFICACION, GENEROS, SEDES_JORNADAS } from '@/lib/institutionalParams';
...
function TeacherBasicInfoContent() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    fechaNacimiento: '', direccion: '', sedeJornada: '',
  });
  ...
  useEffect(() => {
    ...
    if (parsed.length > 0) {
      ...
      const migrated = parsed.map((t: any) => {
        if (t.isActive === undefined) { needsUpdate = true; return { ...t, isActive: true }; }
        return t;
      });
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('edunexus_registered_teachers', JSON.stringify(updated));
    setRegisterSuccess(true);
    setTimeout(() => {
      setShowModal(false);
      setRegisterSuccess(false);
      setForm({ primerNombre: '', segundoNombre: '', primerApellido: '', segundoApellido: '', tipoId: '', numeroId: '', sexo: '', correo: '', telefono: '', celular: '', fechaNacimiento: '', direccion: '', sedeJornada: '' });
  ...
  const filteredTeachers = localTeachers.filter((t: any) => {
    const matchesSearch = t.name.toLowerCase().includes(searchTerm.toLowerCase()) || t.id.includes(searchTerm);
    const matchesActive = includeInactive || t.isActive !== false;
    ...
    <DashboardLayout>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: '14px', marginTop: '4px' }}>Gestión y consulta de datos generales de docentes</p>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          >
            <Plus size={18} /> Registrar Docente
          </button>
        </div>
      </div>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((t: any) => (
                <tr key={t.id} style={{ borderBottom: '1px solid #f3f4f6', transition: 'background 0.2s' }}
                  ...
                  <td style={{ padding: '14px 24px', color: '#2563eb', fontWeight: '700', fontSize: '14px' }}>{t.name.toUpperCase()}</td>
              ...
              <tr>
                <td colSpan={4} style={{ padding: '100px 40px', textAlign: 'center' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div>
                      <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#111827', margin: '0 0 8px 0' }}>No se encontraron docentes</h3>
          ...
          <div style={{ padding: '24px', borderTop: '1px solid #f3f4f6', display: 'flex', justifyContent: 'center' }}>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1}
                style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1px solid #e5e7eb', background: 'white', color: currentPage === 1 ? '#cbd5e1' : '#6b7280', cursor: currentPage === 1 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <ChevronLeft size={18} />
              </button>
              ...
              <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages}
                style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1px solid #e5e7eb', background: 'white', color: currentPage === totalPages ? '#cbd5e1' : '#6b7280', cursor: currentPage === totalPages ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px -10px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ background: 'var(--primary)', padding: '18px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '17px', fontWeight: '900', color: 'white', letterSpacing: '-0.5px' }}>Registrar docente</h2>
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '28px' }}>
              {registerSuccess ? (
                <div style={{ textAlign: 'center', padding: '50px 20px' }}>
                  ...
                  <p style={{ color: '#64748b', margin: 0 }}>El docente ha sido guardado correctamente.</p>
                </div>
              ) : (
...
export default function TeacherBasicInfo() {
  return (
    <Suspense fallback={<div style={{ padding: '40px', textAlign: 'center' }}>Cargando información básica de docentes...</div>}>
      <TeacherBasicInfoContent />
    </Suspense>
  );
}
