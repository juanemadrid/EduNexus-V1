...
const INITIAL_TYPES = [
  ...
  { id: 'ct-2', codigo: 'ECO', nombre: 'Cancelación por Razones Económicas', descripcion: 'Dificultad económica para continuar', estado: 'Activo' },
  { id: 'ct-3', codigo: 'DIS', nombre: 'Cancelación Disciplinaria', descripcion: 'Cancelación por incumplimiento de reglamento', estado: 'Activo' },
  { id: 'ct-4', codigo: 'SAL', nombre: 'Cancelación por Salud', descripcion: 'Motivos de salud del estudiante', estado: 'Activo' },
];

export default function CancellationTypesPage() {
    ...
    <DashboardLayout>
      <div style={{ padding: '0 0 60px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
          <div>
            <h1 style={{ fontSize: '28px', fontWeight: '900', color: '#111827', margin: 0, letterSpacing: '-1px' }}>Tipos de cancelaciones</h1>
            <p style={{ color: '#64748b', fontSize: '14px', marginTop: '4px' }}>Catálogo de tipos de cancelación de matrícula</p>
          </div>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '16px 24px', marginBottom: '20px', display: 'flex', gap: '12px', border: '1px solid #e2e8f0' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            <Search size={16} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
            <input type="text" placeholder="Buscar..." className="input-premium" style={{ paddingLeft: '42px', height: '42px', background: '#f8fafc' }} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={i.id} style={{ borderBottom: '1px solid #f1f5f9' }}
                  ...
                  <td style={{ padding: '14px 24px', fontSize: '12px', color: '#64748b', maxWidth: '320px' }}>{i.descripcion}</td>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(6px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div className="animate-fade" style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '520px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            <div style={{ background: 'var(--primary)', padding: '20px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, color: 'white', fontSize: '18px', fontWeight: '800' }}>{isEditing ? 'Editar tipo' : 'Nuevo tipo de cancelación'}</h2>
            ...
            <div style={{ padding: '32px', display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '20px' }}>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '800', color: '#64748b', marginBottom: '6px', textTransform: 'uppercase' }}>Descripción</label>
                <textarea className="input-premium" value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} style={{ height: '80px', resize: 'vertical', paddingTop: '12px' }} />
              </div>
              <div style={{ gridColumn: 'span 2' }}>
