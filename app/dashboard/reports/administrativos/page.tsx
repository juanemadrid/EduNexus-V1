...
const ALL_FIELDS = [
  ...
  'Nivel académico', 'Escalafón',
  'Especialidad', 'Cargo',
  'Calidad de desempeño', 'Estado',
  'Fecha de ingreso', 'Tiempo laboral',
  'Tipo de vinculación', 'Origen vinculación',
  'Correo institucional', 'Fuente de Recursos',
  'Salario', 'Usuario',
];
...
export default function AdministrativosPage() {
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      alert('Reporte de Administrativos exportado exitosamente.');
    }, 1500);
  };

  // Split fields into two columns
  const half = Math.ceil(ALL_FIELDS.length / 2);
  const leftFields = ALL_FIELDS.slice(0, half);
  const rightFields = ALL_FIELDS.slice(half);
      ...
      <div className="glass-panel" style={{ maxWidth: '900px', margin: '0 auto', background: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)' }}>
        <div style={{ marginBottom: '30px', borderBottom: '1px solid #f1f5f9', paddingBottom: '20px' }}>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>
            Permite exportar a un archivo de Excel la información de los Administrativos registrados en la plataforma, teniendo la posibilidad de seleccionar los campos que requiere en el informe.
          </p>
        </div>
        ...
        <div style={{ padding: '0 20px' }}>
          ...
          {/* Incluir personalizaciones - Toggle */}
            ...
            <div>
              <div
                ...
                style={{
                  ...
                  background: incluirPersonal ? '#10b981' : '#f1f5f9',
                  borderRadius: '12px', cursor: 'pointer',
                  border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center',
                  justifyContent: incluirPersonal ? 'flex-end' : 'flex-start',
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '16px' }}>
                <button
                  onClick={selectAll}
                  style={{ background: 'none', border: 'none', color: '#10b981', fontWeight: '700', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
                >
                  ✓ Todos
                </button>
                <button
                  onClick={selectNone}
                  style={{ background: 'none', border: 'none', color: '#64748b', fontWeight: '700', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
                >
                  □ Ninguno
                </button>
              </div>
