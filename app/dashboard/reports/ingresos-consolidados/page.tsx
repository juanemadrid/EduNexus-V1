...
export default function IngresosConsolidadosPage() {
  const [isLoading, setIsLoading] = useState(false);
  const [form, setForm] = useState({ fechaRango: 'Hoy' });

  const handleCharge = () => {
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      alert('Reporte de Ingresos consolidados exportado exitosamente.');
    }, 1500);
  };
      ...
      <div className="glass-panel" style={{ maxWidth: '850px', margin: '0 auto', background: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)' }}>
        <div style={{ marginBottom: '30px', borderBottom: '1px solid #f1f5f9', paddingBottom: '20px' }}>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>
            Permite visualizar el consolidado de todos los ingresos registrados en el sistema para el rango de fechas seleccionado.
          </p>
        </div>
