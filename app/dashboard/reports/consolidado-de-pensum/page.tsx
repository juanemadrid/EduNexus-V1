...
export default function ConsolidadoPensumPage() {
  const [isLoading, setIsLoading] = useState(false);
  const [form, setForm] = useState({ 
    filtroFecha: 'Período',
    periodo: '2026-01',
    fechaRango: 'Hoy',
    programaId: 'AUXILIAR DE ADMINISTRACION EN SALUD', 
    programaPensum: ''
  });
  const [touched, setTouched] = useState({ 
    periodo: false,
    fechaRango: false,
    programaPensum: false 
  });
  ...
  const handleExport = () => {
    ...
    if (!isPeriodValid || !isDateValid || !form.programaPensum) return;
    
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      alert('Reporte Consolidado de pénsum exportado exitosamente a Excel.');
    }, 1500);
  };
      ...
      <div className="glass-panel" style={{ maxWidth: '850px', margin: '0 auto', background: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)' }}>
         <div style={{ marginBottom: '30px', borderBottom: '1px solid #f1f5f9', paddingBottom: '20px' }}>
           ...
           <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>
             Permite exportar en Excel un archivo de los estudiantes pertenecientes a un programa específico con el estado en que se encuentra cada asignatura.
           </p>
         </div>
         ...
         <div style={{ padding: '0 40px' }}>
           <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 220px) 1fr', gap: '20px', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <div>
                ...
                >
                  <option value="AUXILIAR DE ADMINISTRACION EN SALUD">AUXILIAR DE ADMINISTRACION EN SALUD</option>
                  <option value="AUXILIAR DE ENFERMERÍA">AUXILIAR DE ENFERMERÍA</option>
                </select>
              </div>

              <label style={{ textAlign: 'right', fontSize: '13px', fontWeight: '800', color: isInvalid('programaPensum') ? '#ef4444' : '#334155' }}>
                Programa pénsum <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <div>
                <select 
                  className="input-premium" 
                  style={{ width: '100%', height: '42px', fontSize: '14px', background: '#f8fafc', border: isInvalid('programaPensum') ? '1px solid #ef4444' : '1px solid #e2e8f0' }} 
                  value={form.programaPensum} 
                  onChange={e => { setTouched({ ...touched, programaPensum: true }); handleChange('programaPensum', e.target.value); }}
                >
                  <option value="">Seleccione</option>
                  <option value="Pensum 2024 V1">Pensum 2024 V1</option>
                </select>
                {isInvalid('programaPensum') && <div style={{ color: '#ef4444', fontSize: '11px', marginTop: '6px' }}>El campo es requerido</div>}
              </div>
         ...
         <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #f1f5f9' }}>
            ...
            >
              ...
              {isLoading ? 'Cargando...' : 'Exportar reporte'}
            </button>
         </div>
      </div>
    </DashboardLayout>
