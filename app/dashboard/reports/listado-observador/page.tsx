...
export default function ListadoObservadorPage() {
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      ...
      // For now we just mock an alert. In production it would generate the CSV/PDF
      alert('Reporte de Listado observador exportado exitosamente.');
    }, 1500);
  };
      ...
      <div className="glass-panel" style={{ maxWidth: '850px', margin: '0 auto', background: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)' }}>
         ...
         <div style={{ padding: '0 40px' }}>
           <div style={{ display: 'grid', gridTemplateColumns: 'minmax(100px, 150px) 1fr', gap: '20px', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <select className="input-premium" style={{ width: '100%', height: '42px', fontSize: '14px', background: '#f8fafc' }}>
                <option>Todos</option>
         ...
         <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #f1f5f9' }}>
            <button 
              ...
              style={{ background: '#10b981', color: 'white', display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 24px', fontSize: '14px', fontWeight: '700', opacity: isExporting ? 0.7 : 1, cursor: isExporting ? 'wait' : 'pointer', border: 'none' }}
              onClick={handleExport}
              disabled={isExporting}
            >
              <FileDown size={18} /> 
              {isExporting ? 'Exportando reporte...' : 'Exportar reporte'}
            </button>
         </div>
      </div>
    </DashboardLayout>
