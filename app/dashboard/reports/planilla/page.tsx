...
export default function PlanillaPage() {
  ...
  const handleCharge = () => {
    ...
    if (!isPeriodValid || !isDateValid || !form.sedeJornada || !form.asignaturaId || !form.cursoId) return;
    
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      alert('Reporte de Planilla cargado exitosamente.');
    }, 1500);
  };
      ...
      <div className="glass-panel" style={{ maxWidth: '850px', margin: '0 auto', background: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)' }}>
         <div style={{ marginBottom: '30px', borderBottom: '1px solid #f1f5f9', paddingBottom: '20px' }}>
           ...
           <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>
             Permite obtener el listado de los estudiantes para que los docentes puedan registrar de manera manual notas e inasistencias.
           </p>
         </div>
           ...
           <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 220px) 1fr', gap: '20px', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                {radioOptions.map(option => (
                  <label key={option} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '13px', color: '#475569' }}>
                    <input 
                      type="radio" 
                      name="tipoInforme" 
                      value={option} 
