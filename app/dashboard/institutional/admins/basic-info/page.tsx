            ...
            {currentItems.length > 0 ? currentItems.map((a: any) => (
              <tr key={a.id} style={{ borderBottom: '1px solid #f3f4f6', transition: 'background 0.2s' }}
                ...
                <td style={{ padding: '14px 24px', textAlign: 'center' }}>
                  <Link href={`/dashboard/institutional/admins/basic-info/${a.id}`}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px -10px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ background: 'var(--primary)', padding: '18px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '17px', fontWeight: '900', color: 'white', letterSpacing: '-0.5px' }}>Registrar Administrativo</h2>
            ...
            {activeCount >= MAX_ADMINS && !registerSuccess && (
              <div style={{ margin: '16px 28px 0', padding: '12px 16px', background: '#fef3c7', border: '1px solid #fbbf24', borderRadius: '10px', fontSize: '13px', color: '#92400e', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
                <span>Ha alcanzado el límite máximo de administrativos y no es posible registrar más.</span>
            ...
            {!registerSuccess && (
              <div style={{ padding: '16px 28px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'flex-end', gap: '12px', background: '#fafafa', borderRadius: '0 0 20px 20px' }}>
                <span style={{ marginRight: 'auto', alignSelf: 'center', fontSize: '13px', color: '#6b7280', fontWeight: '600' }}>
                  Admin activos: <strong style={{ color: activeCount >= MAX_ADMINS ? '#ef4444' : 'var(--primary)' }}>{activeCount} / {MAX_ADMINS}</strong>
                </span>
                ...
                <button 
                  ...
                  style={{ padding: '10px 28px', borderRadius: '10px', border: 'none', background: activeCount >= MAX_ADMINS ? '#94a3b8' : 'var(--primary)', color: 'white', fontWeight: '800', cursor: activeCount >= MAX_ADMINS ? 'not-allowed' : 'pointer', fontSize: '14px' }}
                >
                  Aceptar
                </button>
              </div>
