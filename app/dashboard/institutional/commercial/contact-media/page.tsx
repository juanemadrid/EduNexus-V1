...
export default function ContactMediaPage() {
  const [mediaList, setMediaList] = useState<any[]>(() => {
    if (typeof window !== 'undefined') {
      const s = localStorage.getItem('edunexus_registered_contact_media');
  ...
  const handleSave = () => {
    if (!newMediaName.trim()) {
      alert('Debe ingresar un nombre para el medio de contacto.');
      return;
    }
    const newMedia = {
    ...
    localStorage.setItem('edunexus_registered_contact_media', JSON.stringify(updated));
    ...
    <DashboardLayout>
      <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ position: 'relative', flex: 1, maxWidth: '400px' }}>
          ...
          <input 
            type="text" 
            placeholder="Buscar medios contactos..." 
        ...
        >
          <Plus size={16} /> Registrar medio contacto
        </button>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '450px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px -10px rgba(0,0,0,0.3)' }}>
            <div style={{ background: 'var(--primary)', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '16px', fontWeight: '800', color: 'white' }}>Registrar medio contacto</h2>
            ...
            <div style={{ padding: '30px 24px' }}>
              {registerSuccess ? (
                <div style={{ textAlign: 'center', padding: '20px' }}>
                  ...
                  <p style={{ color: '#6b7280', margin: 0, fontSize: '14px' }}>El medio de contacto se ha registrado correctamente.</p>
                </div>
              ) : (
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#374151', marginBottom: '8px' }}>Nombre del medio de contacto *</label>
                  <input 
                    type="text" 
                    className="input-premium" 
                    placeholder="Ej. WhatsApp, Llamada, Referido..."
