...
function BasicInfoContent() {
  ...
  useEffect(() => {
    ...
    if (parsed.length > 0) {
      ...
      const migrated = parsed.map((s: any) => {
        if (s.isActive === undefined) { needsUpdate = true; return { ...s, isActive: true }; }
        return s;
      });
      if (needsUpdate) localStorage.setItem('edunexus_registered_students', JSON.stringify(migrated));
      setLocalStudents(migrated);
    } else {
      const seedData = [
        { id: '113349773746', name: 'MARIA CAROLINA ARROYO CASTILLO', type: 'Estudiante', isActive: true, email: 'maria.arroyo@example.com', residenceCity: 'SINCELEJO', barrio: 'PUERTO ARTURO', address: 'Carrera 15 # 20-30', phone: '2801122', mobile: '3101234567' },
        { id: '10654321', name: 'JUAN EDUARDO MADRID REQUENA', type: 'Estudiante', isActive: true, email: 'juan.madrid@example.com', residenceCity: 'COROZAL', barrio: 'LA ESPERANZA', address: 'Calle 10 # 5-15', phone: '2813344', mobile: '3159876543' }
      ];
      localStorage.setItem('edunexus_registered_students', JSON.stringify(seedData));
      setLocalStudents(seedData);
    }
  }, []);
  ...
  const handleSave = () => {
    ...
    const updated = [...localStudents, newStudent];
