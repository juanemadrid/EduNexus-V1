...
/* Removed INITIAL_PERIODS to keep code clean */

export default function PeriodsPage() {
  const [periods, setPeriods] = useState<Period[]>([]);
  
  const fetchPeriods = async () => {
    const data = await db.list<Period>('academic_periods');
    setPeriods(data);
  };
  ...
  const [itemToDelete, setItemToDelete] = useState<Period | null>(null);
  ...
  const handleEdit = (period: Period) => {
    setForm({
      ...
      inactivationDate: period.inactivationDate || '',
  ...
  const handleDeleteRequest = (period: Period) => {
  ...
  const confirmDelete = async () => {
    ...
    setIsLoading(true);
    await db.delete('academic_periods', itemToDelete.id);
    await fetchPeriods();
  ...
  const handleSave = async () => {
    ...
    if (isEditing && currentId) {
      await db.update('academic_periods', currentId, periodData);
    } else {
      await db.create('academic_periods', periodData);
    }
  ...
  const filtered = periods.filter(p => {
    ...
    const matchesStatus = includeInactive || p.status === 'Activo';
  ...
  const handleExport = () => {
    ...
    const dataToExport = filtered.map(p => ({
      'Ordenamiento': p.order,
      'Período': p.name,
      'Fecha Inicio': formatDate(p.startDate),
      'Fecha Fin': formatDate(p.endDate),
      'Inactivar Estudiantes': p.inactivationDate ? formatDate(p.inactivationDate) : 'No aplica',
      'Estado': p.status
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Períodos");
    XLSX.writeFile(workbook, "Periodos_Academicos.xlsx");
  };
           ...
           <div style={{ paddingTop: '24px', borderTop: '1px solid var(--glass-border)', display: 'flex', gap: '32px', alignItems: 'flex-end' }}>
              <div style={{ flex: 1, paddingBottom: '12px' }}>
                 <label style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', userSelect: 'none' }}>
                    ...
                    />
                    <span style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-main)' }}>¿Incluir períodos inactivos?</span>
                 </label>
              </div>
           </div>
        </div>
              ...
              <tr key={period.id} style={{ borderBottom: '1px solid var(--glass-border)', transition: '0.2s' }} className="table-row-hover">
                ...
                <td style={{ padding: '24px 32px', textAlign: 'right' }}>
                   <div style={{ display: 'flex', gap: '14px', justifyContent: 'flex-end', color: 'var(--text-dim)' }}>
                      ...
                      <button 
                        onClick={() => handleDeleteRequest(period)}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '600px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ background: isEditing ? '#3b82f6' : 'var(--primary)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>{isEditing ? 'Editar período' : 'Crear período'}</h2>
                <p style={{ margin: '4px 0 0', opacity: 0.8, fontSize: '13px', fontWeight: '500' }}>{isEditing ? 'Modifica los detalles del ciclo' : 'Agregue un nuevo ciclo académico al sistema'}</p>
              </div>
            ...
            <div style={{ padding: '32px' }}>
              {success ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                   ...
                   <h3 style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-main)' }}>{isEditing ? '¡Período Actualizado!' : '¡Período Creado!'}</h3>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 4000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '400px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            <div style={{ padding: '32px', textAlign: 'center' }}>
              ...
              <h3 style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '8px' }}>¿Eliminar período?</h3>
              <p style={{ color: 'var(--text-dim)', fontSize: '14px', lineHeight: '1.6' }}>
                Estás a punto de eliminar el período <strong style={{ color: 'var(--text-main)' }}>{itemToDelete?.name}</strong>. Esta acción no se puede deshacer.
              </p>
            </div>
