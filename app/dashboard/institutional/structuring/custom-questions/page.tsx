...
const QUESTION_TYPES = [
  'Texto corto', 'Texto largo', 'Número', 'Fecha', 
  'Selección única', 'Selección múltiple', 'Archivo', 
  'Casilla de verificación', 'Correo electrónico'
];
...
export default function CustomQuestionsPage() {
  ...
  const loadQuestions = async () => {
    ...
    try {
      const data = await db.list<CustomQuestion>('custom_questions');
      setQuestions(data);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing && currentId) {
        await db.update('custom_questions', currentId, questionData);
        setQuestions(questions.map(q => q.id === currentId ? { ...q, ...questionData } : q));
      } else {
        const newId = crypto.randomUUID();
        const newQuestion: CustomQuestion = { ...questionData, id: newId, createdAt: new Date().toISOString() };
        await db.create('custom_questions', newQuestion);
      ...
      <div className="glass-panel" style={{ padding: '0', marginBottom: '32px', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.01)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
             <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: 'var(--text-dim)' }}>
                <input 
                  type="checkbox" 
             ...
             <button 
               onClick={() => { resetForm(); setModalCategory(cat); setShowModal(true); }}
               style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--primary)', fontWeight: '750', fontSize: '13px', background: 'none', border: 'none', cursor: 'pointer' }}
             >
                <Plus size={16} strokeWidth={3} /> Crear pregunta
             </button>
          </div>
        </div>
          ...
          {sectionQuestions.length === 0 ? (
            <div style={{ padding: '48px 32px', textAlign: 'center' }}>
               <p style={{ color: 'var(--text-dim)', fontSize: '14px', fontWeight: '500' }}>
                 No hay registros, verifique los filtros de la consulta o <span onClick={() => { resetForm(); setModalCategory(cat); setShowModal(true); }} style={{ color: 'var(--primary)', fontWeight: '700', cursor: 'pointer', textDecoration: 'underline' }}>cree uno nuevo</span>
               </p>
            </div>
          ) : (
                  ...
                  <tr key={q.id} style={{ borderBottom: '1px solid var(--glass-border)', transition: '0.2s' }} className="table-row-hover">
                    ...
                    <td style={{ padding: '20px 32px', textAlign: 'right' }}>
                       <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', color: 'var(--text-dim)' }}>
                          <button onClick={() => handleEdit(q)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit' }} title="Editar"><Edit size={16} /></button>
                          <button 
                            onClick={async () => {
                              if (confirm('¿Eliminar esta pregunta?')) {
                                setIsLoading(true);
                                try {
                                  await db.delete('custom_questions', q.id);
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '550px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ background: 'var(--primary)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>{isEditing ? 'Editar pregunta' : 'Crear pregunta personalizada'}</h2>
            ...
            <div style={{ padding: '32px' }}>
              {success ? (
                <div style={{ textAlign: 'center', padding: '10px 0' }}>
                   ...
                   <h3 style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-main)' }}>{isEditing ? '¡Pregunta Actualizada!' : '¡Pregunta Creada!'}</h3>
