...
const SECTORS = [
  'Agricultura / Pesca / Ganadería', 'Construcción / obras', 'Educación', 'Energía', 
  'Entretenimiento / Deportes', 'Fabricación', 'Finanzas / Banca', 'Gobierno / No Lucro', 
  'Hostelería / Turismo', 'Informática / Hardware', 'Informática / Software', 'Internet', 
  'Legal / Asesoría', 'Materias Primas', 'Medios de Comunicación', 'Publicidad / RRPP', 
  'RRHH / Personal', 'Salud / Medicina', 'Servicios Profesionales', 'Telecomunicaciones', 
  'Transporte', 'Venta al consumidor', 'Venta al por mayor', 'Mecánica', 'Bienes raíces', 
  'Vigilancia', 'Mecánica diesel', 'Minero', 'Belleza', 'Aviación', 'Veterinaria', 
  'Gastronómico', 'Industrial', 'Textil'
];
const CITIES = [
...
export default function CompaniesPage() {
  ...
  const loadCompanies = async () => {
    ...
    try {
      const data = await db.list<Company>('companies');
      setCompanies(data);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing && currentId) {
        await db.update('companies', currentId, companyData);
        setCompanies(companies.map(c => c.id === currentId ? { ...c, ...companyData } : c));
      } else {
        const newId = crypto.randomUUID();
        const newCompany: Company = { ...companyData, id: newId, createdAt: new Date().toISOString() };
  ...
  const resetForm = () => {
    setForm({ name: '', nit: '', sector: '', phone: '', address: '', city: '', email: '', status: 'Activa' });
    setIsEditing(false);
    setCurrentId(null);
  };
  ...
  const handleExport = () => {
    const headers = ['Nombre', 'NIT', 'Sector', 'Teléfono', 'Dirección', 'Ciudad', 'Email', 'Estado'];
    const rows = filtered.map(c => [
      c.name, c.nit, c.sector, c.phone, c.address, c.city, c.email, c.status
    ]);
    ...
    link.setAttribute("download", `empresas_aliadas_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const filtered = companies.filter(c => {
    const matchesSearch = c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.nit.includes(searchTerm);
    const matchesStatus = includeInactive ? true : c.status === 'Activa';
    ...
    <DashboardLayout>
      <PermissionGate permissionId="inst_estru_perf">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <div>
            <h1 className="heading-premium" style={{ fontSize: '36px', fontWeight: '800', letterSpacing: '-1.5px', margin: 0 }}>Empresas</h1>
            <p style={{ color: 'var(--text-dim)', fontSize: '15px', fontWeight: '500', marginTop: '4px' }}>
              Estructuración institucional / <span style={{ color: 'var(--primary)', fontWeight: '700' }}>Aliados Corporativos</span>
            </p>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            >
              <Plus size={20} strokeWidth={3} /> Crear empresa
            </button>
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'rgba(0,0,0,0.02)', borderBottom: '1px solid var(--glass-border)' }}>
                  {['Empresa', 'NIT', 'Sector', 'Teléfono / Correo', 'Estado', 'Acciones'].map((h, i) => (
              ...
              <tbody>
                {filtered.map((company) => (
                  <tr key={company.id} style={{ borderBottom: '1px solid var(--glass-border)', transition: '0.2s' }} className="table-row-hover">
                    <td style={{ padding: '24px 32px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'rgba(124, 58, 237, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)' }}>
                          <Building2 size={18} />
                        </div>
                        <span style={{ fontWeight: '750', fontSize: '15px', color: 'var(--text-main)' }}>{company.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: '24px 32px', fontSize: '14px', fontWeight: '800', color: 'var(--text-dim)' }}>{company.nit}</td>
                    <td style={{ padding: '24px 32px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-dim)', fontSize: '13px', fontWeight: '600' }}>
                        <Briefcase size={14} /> {company.sector || 'N/A'}
                      </div>
                    </td>
                    <td style={{ padding: '24px 32px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: 'var(--text-main)', fontWeight: '600' }}>
                            <Phone size={14} style={{ color: 'var(--primary)' }} /> {company.phone || '-'}
                        </div>
                    ...
                    <td style={{ padding: '24px 32px' }}>
                      <span style={{ 
                        ...
                        fontSize: '11px', 
                        fontWeight: '800',
                        background: company.status === 'Activa' ? 'rgba(16, 185, 129, 0.12)' : 'rgba(239, 68, 68, 0.12)',
                        color: company.status === 'Activa' ? '#059669' : '#dc2626'
                      }}>
                    ...
                    <td style={{ padding: '24px 32px', textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: '14px', justifyContent: 'flex-end', color: 'var(--text-dim)' }}>
                          <button onClick={() => handleEdit(company)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit' }} title="Editar"><Edit size={18} /></button>
                          <button 
                            onClick={async () => {
                              if (confirm('¿Eliminar esta empresa?')) {
                                setIsLoading(true);
                                try {
                                  await db.delete('companies', company.id);
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '650px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
              ...
              <div style={{ background: 'rgb(34, 197, 94)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
                <div>
                  <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>{isEditing ? 'Editar empresa' : 'Crear empresa'}</h2>
                  <p style={{ margin: '4px 0 0', opacity: 0.8, fontSize: '13px', fontWeight: '500' }}>{isEditing ? 'Actualice la información del aliado' : 'Registre un nuevo aliado empresarial'}</p>
                </div>
              ...
              <div style={{ padding: '32px' }}>
                {success ? (
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    ...
                    <h3 style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-main)' }}>{isEditing ? '¡Empresa Actualizada!' : '¡Empresa Registrada!'}</h3>
                    <p style={{ color: 'var(--text-dim)', marginTop: '4px' }}>La información ha sido guardada exitosamente.</p>
                  </div>
                ) : (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div style={{ gridColumn: 'span 2' }}>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: 'var(--text-dim)', marginBottom: '8px', textTransform: 'uppercase' }}>Empresa *</label>
                      <input 
                        className="input-premium" 
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '800', color: 'var(--text-dim)', marginBottom: '8px', textTransform: 'uppercase' }}>Sector empresarial</label>
                      <select 
                        className="input-premium" 
