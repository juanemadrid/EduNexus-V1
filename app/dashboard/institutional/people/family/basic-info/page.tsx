...
function FamilyBasicInfoContent() {
  ...
  const loadFamily = async () => {
    ...
    try {
      const data = await db.list<any>('family');
      setLocalFamily(data);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.create('family', newFamiliar);
      setLocalFamily(prev => [...prev, newFamiliar]);
      ...
      alert('Error al registrar el familiar.');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              currentItems.map((s: any) => (
                <tr key={s.id} style={{ borderBottom: '1px solid #f3f4f6', transition: 'background 0.2s' }}
                  ...
                  <td style={{ padding: '14px 24px', textAlign: 'center' }}>
                    <Link href={`/dashboard/institutional/people/family/basic-info/${s.id}`}>
