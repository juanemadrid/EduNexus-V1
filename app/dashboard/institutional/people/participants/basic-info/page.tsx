...
function ParticipantsBasicInfoContent() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    fechaNacimiento: '', direccion: '', tipoParticipante: '', institucion: ''
  });
  ...
  const loadParticipants = async () => {
    ...
    try {
      const data = await db.list<any>('participants');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newParticipant = {
        ...
        type: 'Participante',
        tipoParticipante: form.tipoParticipante || 'Externo',
        isActive: true,
        correo: form.correo,
      ...
      await db.create('participants', newParticipant);
      ...
      setTimeout(() => {
        ...
        setForm({ primerNombre: '', segundoNombre: '', primerApellido: '', segundoApellido: '', tipoId: '', numeroId: '', sexo: '', correo: '', telefono: '', celular: '', fechaNacimiento: '', direccion: '', tipoParticipante: '', institucion: '' });
    ...
    <DashboardLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: '14px', marginTop: '4px' }}>Gestión y consulta de datos generales de participantes</p>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          >
            <Plus size={18} /> Registrar Participante
          </button>
        </div>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '24px', marginBottom: '24px', background: 'white' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            <input type="text" placeholder="Buscar participantes..." className="input-premium"
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #e5e7eb', background: '#f9fafb' }}>
              ...
              <th style={{ textAlign: 'left', padding: '16px 24px', fontSize: '12px', color: '#6b7280', fontWeight: '800', textTransform: 'uppercase' }}>Tipo Participante</th>
              ...
              currentItems.map((s: any) => (
                <tr key={s.id} style={{ borderBottom: '1px solid #f3f4f6', transition: 'background 0.2s' }}
                  ...
                  <td style={{ padding: '14px 24px', textAlign: 'center' }}>
                    <Link href={`/dashboard/institutional/people/participants/basic-info/${s.id}`}>
              ...
              <tr>
                <td colSpan={4} style={{ padding: '100px 40px', textAlign: 'center' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div>
                      <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#111827', margin: '0 0 8px 0' }}>No se encontraron participantes</h3>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px -10px rgba(0,0,0,0.3)' }}>
            <div style={{ background: 'var(--primary)', padding: '18px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '17px', fontWeight: '900', color: 'white', letterSpacing: '-0.5px' }}>Registrar participante</h2>
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '28px' }}>
              {registerSuccess ? (
                <div style={{ textAlign: 'center', padding: '50px 20px' }}>
                  ...
                  <p style={{ color: '#64748b', margin: 0 }}>El participante ha sido guardado correctamente.</p>
                </div>
              ) : (
...
export default function ParticipantsBasicInfoPage() {
  return (
    <Suspense fallback={<div style={{ padding: '40px', textAlign: 'center' }}>Cargando información básica de participantes...</div>}>
      <ParticipantsBasicInfoContent />
    </Suspense>
  );
}
