...
export default function AdminProfileView() {
  ...
  const loadAdmin = async () => {
    ...
    try {
      const data = await db.get<any>('admins', adminId);
      if (data) {
        setLocalAdmin(data);
        if (data.roles) setAdminRoles(data.roles);
        if (data.profileImage) setProfileImage(data.profileImage);
        if (data.firmaImage) setFirmaImage(data.firmaImage);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      const updatedData = {
        ...
        details: { ...(admin.details || {}), ...editableData }
      };
      await db.update('admins', adminId, updatedData);
      setLocalAdmin((prev: any) => ({ ...prev, ...updatedData }));
      setIsEditing(false);
    } catch (error) {
    ...
    return (
      <DashboardLayout>
        <div style={{ padding: '80px', textAlign: 'center', color: '#64748b' }}>
          <p style={{ fontWeight: '600' }}>Cargando perfil del administrativo...</p>
        </div>
      </DashboardLayout>
    );
  }
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 500 * 1024) {
        alert('La imagen no debe superar los 500KB para ser almacenada correctamente.');
        return;
      }
      ...
      reader.onloadend = async () => {
        ...
        try {
          await db.update('admins', adminId, { profileImage: base64 });
        } catch (error) {
  ...
  const handleDeleteAdmin = async () => {
    ...
    try {
      await db.delete('admins', adminId);
            ...
            <div style={{ padding: '12px 24px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              ...
              <button 
                onClick={async () => {
                  if (firmaImage) {
                    try {
                      await db.update('admins', adminId, { firmaImage });
                    } catch (error) {
