...
export default function ParticipantProfileView() {
  ...
  React.useEffect(() => {
    ...
    if (saved) setLocalParticipants(JSON.parse(saved));
  }, []);

  const allParticipants = [...localParticipants];
  const participant = allParticipants.find(p => p.id === participantId) || allParticipants[0];

  if (!participant) {
    return (
      <DashboardLayout>
        <div style={{ padding: '40px', textAlign: 'center' }}>
           <h2 style={{ fontWeight: '800' }}>Participante no encontrado</h2>
  ...
  const infoGrid = [
    { label: 'Nombres', value: (participant.details?.primerNombre || '') + ' ' + (participant.details?.segundoNombre || '') },
    { label: 'Apellidos', value: (participant.details?.primerApellido || '') + ' ' + (participant.details?.segundoApellido || '') },
    { label: 'Identificación', value: `${participant.details?.tipoId || 'C.C.'} ${participant.id}` },
    { label: 'Sexo', value: participant.details?.sexo || 'N/A' },
    { label: 'Teléfono', value: participant.details?.telefono || 'N/A' },
    { label: 'Celular', value: participant.details?.celular || 'N/A' },
    { label: 'Correo electrónico', value: participant.correo || 'N/A' },
    { label: 'Dirección', value: participant.details?.direccion || 'N/A' },
    { label: 'Lugar de residencia', value: participant.details?.residence || 'N/A' },
    { label: 'Barrio', value: participant.details?.barrio || 'N/A' },
    { label: 'Fecha de nacimiento', value: participant.details?.fechaNacimiento || 'N/A' },
    { label: 'Lugar de nacimiento', value: participant.details?.lugarNacimiento || 'N/A' },
  ];
    ...
    <DashboardLayout>
      ...
      <div className="glass-panel" style={{ padding: '32px', marginBottom: '24px' }}>
        <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
          ...
          <div>
             <h1 style={{ fontSize: '28px', fontWeight: '800', margin: 0 }}>{participant.name?.toUpperCase()}</h1>
             <p style={{ color: 'var(--text-dim)', fontSize: '15px', fontWeight: '600', marginTop: '4px' }}>
                {participant.details?.tipoId || 'C.C.'} {participant.id} • {participant.tipoParticipante || 'Participante'}
             </p>
          </div>
        </div>
      </div>
