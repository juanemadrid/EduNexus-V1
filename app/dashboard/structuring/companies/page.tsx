...
const SECTORS = ['Educativo', 'Tecnología', 'Salud', 'Servicios', 'Comercio', 'Industrial'];
const CITIES = ['Sincelejo', 'Corozal', 'Morroa', 'Sampues', 'Los Palmitos', 'Barrio'];

export default function CompaniesPage() {
  const [companies, setCompanies] = useState<Company[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('edunexus_companies');
  ...
  useEffect(() => {
    localStorage.setItem('edunexus_companies', JSON.stringify(companies));
  }, [companies]);
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 className="heading-premium" style={{ fontSize: '36px', fontWeight: '800', letterSpacing: '-1.5px', margin: 0 }}>Gestión de Empresas</h1>
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: '60px 32px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-dim)', fontSize: '15px', fontWeight: '500' }}>
               No hay registros, verifique los filtros de la consulta o <span onClick={() => setShowModal(true)} style={{ color: 'var(--primary)', fontWeight: '700', cursor: 'pointer', textDecoration: 'underline' }}>cree uno nuevo</span>
            </p>
          </div>
        ) : (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '650px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ background: 'rgb(34, 197, 94)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>Crear empresa</h2>
                <p style={{ margin: '4px 0 0', opacity: 0.8, fontSize: '13px', fontWeight: '500' }}>Registre un nuevo aliado empresarial</p>
              </div>
            ...
            <div style={{ padding: '32px' }}>
              {success ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                   ...
                   <h3 style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-main)' }}>¡Empresa Registrada!</h3>
