...
const INITIAL_DATA: BranchShift[] = [
  { id: '1', branch: 'Principal', shift: 'Mañana' },
  { id: '2', branch: 'Principal', shift: 'Tarde' },
];
...
export default function BranchesShiftsPage() {
  const [data, setData] = useState<BranchShift[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('edunexus_branch_shifts');
  ...
  useEffect(() => {
    localStorage.setItem('edunexus_branch_shifts', JSON.stringify(data));
  }, [data]);

  const handleSave = () => {
    if (!form.branch || !form.shift) {
              ...
              <tr key={item.id} style={{ borderBottom: '1px solid var(--glass-border)', transition: '0.2s' }} className="table-row-hover">
                <td style={{ padding: '24px 32px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <span style={{ fontWeight: '750', fontSize: '15px', color: 'var(--text-main)' }}>{item.branch}</span>
                  </div>
                </td>
                <td style={{ padding: '24px 32px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--primary)' }} />
                    <span style={{ fontWeight: '600', fontSize: '15px', color: 'var(--text-dim)' }}>{item.shift}</span>
                  </div>
                </td>
                <td style={{ padding: '24px 32px', textAlign: 'right' }}>
                   <div style={{ display: 'flex', gap: '14px', justifyContent: 'flex-end', color: 'var(--text-dim)' }}>
                      ...
                      <button 
                        onClick={() => {
                          if (confirm('¿Eliminar esta combinación de sede y jornada?')) {
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '540px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ background: 'var(--primary)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>Crear Sede - jornada</h2>
                <p style={{ margin: '4px 0 0', opacity: 0.8, fontSize: '13px', fontWeight: '500' }}>Configura la disponibilidad horaria por establecimiento</p>
              </div>
