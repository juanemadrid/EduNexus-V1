...
export default function DigitalDocumentsPage() {
  const [requirements, setRequirements] = useState<DocRequirement[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('edunexus_doc_requirements');
      return saved ? JSON.parse(saved) : [
        { id: '1', name: 'FOTOCOPIA DOCUMENTO IDENTIDAD', category: 'DOCUMENTOS ACADÉMICOS', isRequired: true, status: 'Activo' },
        { id: '2', name: 'FOTOCOPIA CARNET EPS', category: 'DOCUMENTOS ACADÉMICOS', isRequired: true, status: 'Activo' },
        { id: '3', name: 'DIPLOMA DE BACHILLER', category: 'DOCUMENTOS ACADÉMICOS', isRequired: true, status: 'Activo' },
      ];
    }
    return [];
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedCats = localStorage.getItem('edunexus_digital_categories');
      if (savedCats) {
  ...
  useEffect(() => {
    localStorage.setItem('edunexus_doc_requirements', JSON.stringify(requirements));
  }, [requirements]);
