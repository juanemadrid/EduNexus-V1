...
export default function EducationalInstitutionsPage() {
  const [institutions, setInstitutions] = useState<Institution[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('edunexus_educational_institutions');
  ...
  useEffect(() => {
    localStorage.setItem('edunexus_educational_institutions', JSON.stringify(institutions));
  }, [institutions]);
    ...
    <DashboardLayout>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          >
            <Plus size={20} strokeWidth={3} /> Crear institución
          </button>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '650px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ background: 'rgb(34, 197, 94)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800' }}>Crear institución educativa</h2>
                <p style={{ margin: '4px 0 0', opacity: 0.8, fontSize: '13px', fontWeight: '500' }}>Registre un nuevo aliado educativo en el sistema</p>
              </div>
            ...
            <div style={{ padding: '32px' }}>
              {success ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                   ...
                   <h3 style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-main)' }}>¡Institución Registrada!</h3>
