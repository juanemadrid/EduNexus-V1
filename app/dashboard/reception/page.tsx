...
export default function ReceptionAccessControl() {
  ...
  const fetchData = async () => {
    try {
      // Load Students
      const savedStudents = await db.list<any>('students');
      setStudents(savedStudents);

      // Load Configs (Cursos, Grupos, Productos)
      // These could also be in Firestore, but for now we prioritized Students and Logs
      ...
      const savedProductos = JSON.parse(localStorage.getItem('edunexus_productos') || '[]');
      setProductos(savedProductos);

      const savedLogs = await db.list<AccessLog>('campus_access_log');
