...
import { 
  ...
  ChevronDown, Search, Plus, UserPlus, X, Check, Trash2
} from 'lucide-react';
...
export default function StudentProfileView() {
  ...
  React.useEffect(() => {
    const savedData = localStorage.getItem('edunexus_registered_students');
    if (savedData) {
      const parsed = JSON.parse(savedData);
      setLocalStudents(parsed);
      const studentData = parsed.find((s: any) => s.id === studentId);
      if (studentData?.photo) {
        setProfileImage(studentData.photo);
      }
    }
  }, [studentId]);
  ...
  useEffect(() => {
    // Load links
    const savedLinks = localStorage.getItem('edunexus_student_family_links');
    if (savedLinks) {
      const allLinks = JSON.parse(savedLinks);
      setLinks(allLinks.filter((l: any) => l.studentId === studentId));
    }

    // Load family members for search
    const savedFamily = localStorage.getItem('edunexus_registered_family');
    if (savedFamily) setAvailableFamily(JSON.parse(savedFamily));
  }, [studentId]);

  const handleCreateLink = (familiar: any) => {
    const newLink = {
    ...
    localStorage.setItem('edunexus_student_family_links', JSON.stringify(updated));
    setLinks(updated.filter((l: any) => l.studentId === studentId));
  ...
  const handleUnlink = (linkId: string) => {
    ...
    const savedLinks = localStorage.getItem('edunexus_student_family_links');
    const allLinks = savedLinks ? JSON.parse(savedLinks) : [];
    const updated = allLinks.filter((l: any) => l.id !== linkId);
  ...
  const saveStudentData = (data: any, newPhoto?: string) => {
    const updatedStudents = localStudents.map(s => {
      if (s.id === studentId) {
        return { 
          ...s, 
          name: `${data.nombres} ${data.apellidos}`.trim(),
          photo: newPhoto !== undefined ? newPhoto : s.photo,
          // Could add more fields here but for demo let's stay focused
        };
      }
      return s;
    });
    setLocalStudents(updatedStudents);
    localStorage.setItem('edunexus_registered_students', JSON.stringify(updatedStudents));
  };

  const handleSavePhoto = () => {
    if (profileImage) {
      saveStudentData(editableData, profileImage);
