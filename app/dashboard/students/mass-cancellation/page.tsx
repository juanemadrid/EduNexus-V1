...
import { Search, ChevronDown, UserX, AlertCircle, Trash2, X, Save, Info, CheckSquare, Square } from 'lucide-react';
  ...
  const handleConsultar = () => {
    ...
    setTimeout(() => {
      ...
      const filtered = localStudents.filter(s => 
        (s.isEnrolled !== false) && 
        (s.isActive !== false) &&
        (filters.program === 'Seleccione' || s.details?.program === filters.program)
      );
               ...
               <tbody>
                 {matchingStudents.map(s => (
                   <tr key={s.id} onClick={() => toggleSelect(s.id)} style={{ borderBottom: '1px solid #f8fafc', cursor: 'pointer', background: selectedIds.includes(s.id) ? '#fff1f2' : 'transparent' }}>
                     ...
                     <td style={{ padding: '16px 24px', fontSize: '13px', color: '#64748b' }}>{s.details?.program || 'Bachillerato Académico'}</td>
                   </tr>
                 ))}
               </tbody>
            </table>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', zIndex: 3500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '550px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px -10px rgba(0,0,0,0.3)' }}>
            <div style={{ background: '#ef4444', padding: '18px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               ...
               <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}><X size={22} /></button>
            </div>
            ...
            <div style={{ padding: '28px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div>
                  ...
                  <input type="date" className="input-premium" style={{ width: '100%', height: '40px' }} value={cancellationData.fecha} onChange={e => setCancellationData({...cancellationData, fecha: e.target.value})} />
                </div>
                <div>
                  ...
                  <select className="input-premium" style={{ width: '100%', height: '40px' }} value={cancellationData.motivo} onChange={e => setCancellationData({...cancellationData, motivo: e.target.value})}>
            ...
            <div style={{ padding: '16px 28px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'flex-end', gap: '12px', background: '#fafafa' }}>
               <button onClick={() => setShowModal(false)} style={{ padding: '10px 24px', borderRadius: '10px', background: 'white', border: '1px solid #e2e8f0', color: '#64748b', fontWeight: '700', cursor: 'pointer' }}>Cerrar</button>
               <button onClick={confirmCancellation} style={{ padding: '10px 32px', borderRadius: '10px', background: '#ef4444', color: 'white', fontWeight: '800', cursor: 'pointer', border: 'none', display: 'flex', alignItems: 'center', gap: '8px' }}>
