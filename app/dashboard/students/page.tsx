  ...
  return (
    <DashboardLayout>
      ...
      <div className="glass-panel" style={{ padding: '0', overflow: 'hidden' }}>
        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'flex-end' }}>
           <select 
                ...
                <td style={{ padding: '22px 32px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: '12px', color: 'var(--text-dim)', fontWeight: '500' }}>{student.email}</p>
                    </div>
                  </div>
                </td>
                <td style={{ padding: '22px 32px', fontSize: '14px', fontWeight: '600', color: 'var(--text-main)' }}>{student.level}</td>
                <td style={{ padding: '22px 32px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '120px', height: '8px', background: '#f1f5f9', borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.03)' }}>
                      <div style={{ width: `${(student.gpa / 5) * 100}%`, height: '100%', background: 'var(--primary)', boxShadow: '0 0 10px rgba(16, 185, 129, 0.4)' }}></div>
                    </div>
                    <span style={{ fontWeight: '800', fontSize: '14px', color: 'var(--primary)' }}>{student.gpa}</span>
                  </div>
                </td>
                <td style={{ padding: '22px 32px' }}>
                  <span style={{ 
                    ...
                    fontSize: '11px', 
                    fontWeight: '800',
                    background: student.status === 'active' ? 'rgba(16, 185, 129, 0.12)' : student.status === 'suspended' ? 'rgba(245, 158, 11, 0.12)' : 'rgba(239, 68, 68, 0.12)',
                    color: student.status === 'active' ? '#059669' : student.status === 'suspended' ? '#d97706' : '#dc2626',
